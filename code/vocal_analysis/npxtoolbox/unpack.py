...
## response data reconstructor files
def pull_data_from_recondat(df_in, dir= './../../../Npv1_data/Npv1_audio_log_data'):
     # df to cycle through
     # directory: relative path by default, change to hard if needed
     #            should end one step up from animal groups
     
     df = df_in.copy()
     
     df['orig_trig_start_seconds'] = np.nan
     df['seg_len_seconds'] = np.nan
     df['stim_dur_seconds'] = np.nan

     recon_f_prev = None # to check if you need to load new data

     for i in range(len(df)):
          rowi = df.iloc[i].copy()
          
          # using known structure, find relative reconstuctor file
          rel_reconstructor_f = glob(dir+
                                   '/*'+rowi['anim']+ 
                                   '/*sess-'+str(rowi['sess_n'])+'_'+rowi['treatment']+
                                   '/das'+
                                   '/*'+rowi['condition']+'*'+str(rowi['rec_iter'])+'_reconstructor.csv')

          if len(rel_reconstructor_f)==1:
               recon_f = rel_reconstructor_f[0]
               if recon_f == recon_f_prev:
                    recon_dat = recon_dat
               else:
                    recon_dat = pd.read_csv(recon_f)

          elif len(rel_reconstructor_f)==0:
               print('Error: no relative reconstructor file found! See data:')
               print(rowi)
               break
          else: 
               print('Error: more than one relative reconstructor file found! See data:')
               print(rowi)
               break
          
          if ((rowi['anim']=='M2') and 
               (rowi['sess_n']==1) and
               (rowi['condition']=='echo_seq') and
               (rowi['rec_iter']==2)):
             rowi['trial_n'] = rowi['trial_n']-20
          if ((rowi['anim']=='M2') and 
               (rowi['sess_n']==1) and
               (rowi['condition']=='echo_seq') and
               (rowi['rec_iter']==3)):
             rowi['trial_n'] = rowi['trial_n']-40


          if (recon_dat['trial_#']==int(rowi['trial_n'])).sum()==0:
               print('Trial not found in reconstructor file.')
               print(rowi)
               print(recon_dat)
               break

          reconi = recon_dat.loc[recon_dat['trial_#']==int(rowi['trial_n'])] 

          # values to extract and add to df
          dat_to_add = dict(zip(['orig_trig_start_seconds','seg_len_seconds','stim_dur_seconds'],
                    [reconi.orig_trig_start_seconds.values[0],
                    reconi.seg_lens_seconds.values[0],
                    (reconi.orig_seg_start_seconds-reconi.orig_trig_start_seconds).values[0]]))
          
          df.iloc[i,-3] = dat_to_add['orig_trig_start_seconds']
          df.iloc[i,-2] = dat_to_add['seg_len_seconds']
          df.iloc[i,-1] = dat_to_add['stim_dur_seconds']

          # before exiting loop
          recon_f_prev = recon_f
